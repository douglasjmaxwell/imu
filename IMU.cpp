...
void IMU::selectDevice(int file, int addr)
{
  if (ioctl(file, I2C_SLAVE, addr) < 0) {
    cout << "Failed to select I2C device." << endl;
  }
}


void IMU::readACC(int  a[])
{
  uint8_t block[6];
  if (BerryIMUversion == 1){
...
void IMU::writeAccReg(uint8_t reg, uint8_t value)
{
  ...
  if (result == -1){
    cout << "Failed to write byte to I2C Acc." << endl;
    exit(1);
  }
}
...
void IMU::writeMagReg(uint8_t reg, uint8_t value)
{
  ...
  int result = i2c_smbus_write_byte_data(file, reg, value);
  if (result == -1){
    cout << "Failed to write byte to I2C Mag." << endl;
    exit(1);
  }
}
...
void IMU::writeGyrReg(uint8_t reg, uint8_t value)
{
  ...
  int result = i2c_smbus_write_byte_data(file, reg, value);
  if (result == -1){
    cout << "Failed to write byte to I2C Gyr." << endl;
    exit(1);
  }
}
...
void IMU::enableIMU()
{
  ...
  if (BerryIMUversion == 3){//For BerryIMUv3
    ...
    // Enable the accelerometer
    writeAccReg(LSM6DSL_CTRL1_XL,0b10011111);       // ODR 3.33 kHz, +/- 16g , BW = 400hz
